...
void runJetAnalyzer::makeStabilityHists()
{
	...
	TH1F* hrun = new TH1F("hrun","hrun",2000,211000.5,213000.5);
  ...
  cout << scientific;
	...
	TH1F* hj1pt [numtrgs][nybins-1]; TH1F* hj2pt [numtrgs][nybins-1];
	TH1F* hj1phi[numtrgs][nybins-1]; TH1F* hj2phi[numtrgs][nybins-1];
	TH1F* hj1chf[numtrgs][nybins-1]; TH1F* hj2chf[numtrgs][nybins-1];
	TH1F* hj1nhf[numtrgs][nybins-1]; TH1F* hj2nhf[numtrgs][nybins-1];
  TH1F* hmet  [numtrgs][nybins-1];
   ...
   unsigned nentries(runningOnIncxsec?treeak7->fChain->GetEntries()
			:ak5Analyzer->fChain->GetEntries());
...
//		nentries = 1000;
	...
	for(unsigned jentry(0);jentry<nentries;++jentry) {
      ...
      treeak5->fChain->GetEntry(jentry);
		...
		{
			ak5Analyzer->fChain->GetEntry(jentry);
			hltAnalyzer->fChain->GetEntry(jentry);
			skimAnalyzer->fChain->GetEntry(jentry);
			metAnalyzer->fChain->GetEntry(jentry);
		}	
		//find the run index-------------------------------
		unsigned runIndex(0);
		for(unsigned rn(0);rn<numRuns && !runOnMC;++rn)
			if(runNumbers[rn]==hltAnalyzer->Run)
				runIndex=rn+1;
		//		 assert(runIndex);
		//found the run index------------------------------
		...
		for(unsigned ntrg(0);ntrg<numtrgs;++ntrg)  
			if(runningOnIncxsec)
				...
				prescl[ntrg] = (*(hltPsc[ntrg]))*(*(l1Psc[ntrg])); //psc with l1pass 
		...
		for(unsigned ntrg(0);ntrg<numtrgs;++ntrg)  
			if((*(hltPass[ntrg]))>0.5)     {      //hlt pass 
				...
				for(unsigned njet(0);njet<(*nref);++njet ) {
					...
					Jet jet(jtpt[njet],jteta[njet],jtphi[njet],
							jtm[njet]);
					const double energy =  jet.getLorentzVector().Energy();
					const double et     =  rawpt[njet];
					chf = chargedHardSum[njet]/et;
					nhf = neutralHardSum[njet]/et;
					muf = muSum[njet]/et;
					elf = eSum[njet]/et;
					phf = photonSum[njet]/et;
					...
					cut =
						...
						nhf < 0.9   &&  elf < 0.9	&&  
						phf < 0.9	  &&  
						skimAnalyzer->pPAcollisionEventSelectionPA>0.5&&
						skimAnalyzer->pHBHENoiseFilter>0.5&&
						trackN[njet]>4
						;
					if(cut) 
						for(unsigned i(0);i<nybins-1;++i) ///Cycle over eta bins 
							if(abs(jty[njet])>=ybins[i] && abs(jty[njet])<ybins[i+1]) {
								++jetsThisEvent[i];
                ...
                hmet[ntrg][i]->Fill((*met)/(*sumET),wt);
								if(njet==0) { ///leading jet
									if(!runOnMC) {
										...
										Jet1chf[ntrg][i]->Fill(runIndex,chf,1); 
										Jet1nhf[ntrg][i]->Fill(runIndex,nhf,1); 
										...
										hj1chf[ntrg][i]->Fill(chf,wt);
										hj1nhf[ntrg][i]->Fill(nhf,wt);
								}
								else if(njet==1) {///second jet
									if(!runOnMC) {
										Jet2pt [ntrg][i]->Fill(runIndex,jtpt[njet],1);    
										Jet2chf[ntrg][i]->Fill(runIndex,chf,1); 
										Jet2nhf[ntrg][i]->Fill(runIndex,nhf,1); 
										...
										hj2chf[ntrg][i]->Fill(chf,wt);
										hj2nhf[ntrg][i]->Fill(nhf,wt);
								}
							}///eta loop
				}//for loop on jets
