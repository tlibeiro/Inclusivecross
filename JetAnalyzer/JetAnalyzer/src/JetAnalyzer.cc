...
class JetAnalyzer : public edm::EDAnalyzer {
   ...
   private:
      ...
      virtual void beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&);
			...
			const bool runOnMC;
      const bool saveHLTInfo, useak5Corr;
...
JetAnalyzer::JetAnalyzer(const edm::ParameterSet& iConfig)
	...
	saveHLTInfo(iConfig.getUntrackedParameter<bool>("saveHLTInfo",true)),
	useak5Corr(iConfig.getUntrackedParameter<bool>("useak5Corr",false)),
	...
	iC(iConfig),outTree(0)
{
	...
	outTree->Branch("nref", &numGoodJets, "nref/I");
	outTree->Branch("jtpt" ,jetpt       ,("jtpt[" +njets+"]/F").c_str());
	outTree->Branch("jteta",jeteta      ,("jteta["+njets+"]/F").c_str());
	outTree->Branch("jtphi",jetphi      ,("jtphi["+njets+"]/F").c_str());
	outTree->Branch("jtm"  ,jetm        ,("jtm["  +njets+"]/F").c_str());
	outTree->Branch("jty"  ,jety        ,("jty["  +njets+"]/F").c_str());
	outTree->Branch("rawpt",jetpt_uncorr,("rawpt["+njets+"]/F").c_str());
	outTree->Branch("jecl1"  ,jecl1      ,("jecl1["  +njets+"]/F").c_str());
	outTree->Branch("jecl2l3",jecl2l3    ,("jecl2l3["+njets+"]/F").c_str());
	outTree->Branch("jecres" ,jecres     ,("jecres[" +njets+"]/F").c_str());
	outTree->Branch("chargedHardSum",chargedHardSum,("chargedHardSum["+njets+"]/F").c_str());
	outTree->Branch("chargedHardN"  ,chargedHardN  ,("chargedHardN["  +njets+"]/F").c_str());
	outTree->Branch("neutralHardSum",neutralHardSum,("neutralHardSum["+njets+"]/F").c_str());
	outTree->Branch("neutralHardN"  ,neutralHardN  ,("neutralHardN["  +njets+"]/F").c_str());
	outTree->Branch("emEnergy"      ,emEnergy      ,("emEnergy["      +njets+"]/F").c_str());
	outTree->Branch("chemEnergy"    ,chemEnergy    ,("chemEnergy["    +njets+"]/F").c_str());
	outTree->Branch("nuemEnergy"    ,nuemEnergy    ,("nuemEnergy["    +njets+"]/F").c_str());
	outTree->Branch("muSum"         ,muSum         ,("muSum["         +njets+"]/F").c_str());
	outTree->Branch("eSum"          ,eSum          ,("eSum["          +njets+"]/F").c_str());
	outTree->Branch("photonSum"     ,photonSum     ,("photonSum["     +njets+"]/F").c_str());
	outTree->Branch("trackN"        ,trackN        ,("trackN["        +njets+"]/I").c_str());
	outTree->Branch("constituentN"  ,constituentN  ,("constituentN["  +njets+"]/I").c_str());
	if(runOnMC) {
		outTree->Branch("matchgenpt",matchGenPt,("matchgenpt["  +njets+"]/F").c_str());
		outTree->Branch("matchgendr",matchGendr,("matchgendr["  +njets+"]/F").c_str());
  ...
  string jettype("AK7PFchs");
 if(useak5Corr) jettype = "AK5PFchs";
//	jecStr.push_back( fDir + "_L1FastJet_"+jettype+".txt" );
	jecStr.push_back( fDir + "_L2Relative_"+jettype+".txt" );
	jecStr.push_back( fDir + "_L3Absolute_"+jettype+".txt" );
	if (!runOnMC)
		jecStr.push_back( fDir + "_L2L3Residual_"+jettype+".txt" );
...
// ------------ helper functions  ------------
const TLorentzVector JetAnalyzer::getCorrectedJet(const reco::PFJet& jet) {
	double jecVal = 1.0;
...
//
void JetAnalyzer::getSubCorr(const reco::PFJet& jet, double* subc) {
	//initialize array
...
double JetAnalyzer::getJEC(double curJetEta,
		double curJetPt,
...
// ------------ method called when starting to processes a run  ------------
	void 
JetAnalyzer::beginRun(edm::Run const& run, edm::EventSetup const& setup)
{
...
double JetAnalyzer::analyzeTrigger(const edm::Event& iEvent, const edm::EventSetup& iSetup, const std::string& triggerName) {
